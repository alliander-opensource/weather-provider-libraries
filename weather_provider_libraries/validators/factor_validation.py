...
from weather_provider_libraries.factor_data.eccodes_factor_listing import KNOWN_ECCODES_FACTORS_BY_ID
...
def validate_eccodes_factors_list_by_id_code(potential_factors: list[int]) -> bool:
    """This is a method aimed at validating a list of eccodes factors by their suggested id code.

    A boolean is returned containing the result of this evaluation.
    ...
    return all(i in KNOWN_ECCODES_FACTORS_BY_ID.keys() for i in potential_factors)
...
def split_valid_and_invalid_eccodes_factors_list_by_id_code(
    ...
    if filter_list is None:
        list_of_existing_factors = [
            factor for factor in potential_factors if factor in KNOWN_ECCODES_FACTORS_BY_ID.keys()
        ]
        list_of_non_existing_factors = [
            factor for factor in potential_factors if factor not in KNOWN_ECCODES_FACTORS_BY_ID.keys()
        ]
    else:
        list_of_existing_factors = [
            factor
            for factor in potential_factors
            if factor in filter_list and factor in KNOWN_ECCODES_FACTORS_BY_ID.keys()
        ]
        list_of_non_existing_factors = [
            factor
            for factor in potential_factors
            if factor not in list_of_existing_factors or factor not in KNOWN_ECCODES_FACTORS_BY_ID.keys()
        ]
