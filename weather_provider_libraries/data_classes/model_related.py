...
class ModelIdentity(BaseModel):
    ...
    """
    id: str = Field(
        min_length=4, max_length=12, title="Model Identifier",
        description="Short string identifier of the model. Lower case letters only",
        pattern="^[a-z]+$"
    )
    name: str = Field(
        min_length=8, max_length=32, title="Model Name",
        description="Short identifying name of the model. Letters only",
        pattern="^[a-zA-Z ]+$"
    )
    description: str = Field(
        min_length=12, max_length=255, title="Model Description",
    ...
    license_information: str = Field(
        min_length=2, max_length=255, title="Model Licensing Information",
...
class ModelProperties(BaseModel):
    ...
    def metadata(self) -> dict[str, str]:
        ...
        return {
            ...
            "Storage Mode": self.storage_mode_to_use
        }
...
class ModelGeography(BaseModel):
    ...
    area_bounding_box: BBox | None = Field(
        default=None, title="Model Area Bounding Box",
        description="A PyProj BBox object representing the outer bounds of the data covered by this model's data source"
    )
    ...
    source_crs: CRS = Field(
        default=CRS("EPSG:4326"), title="Model CRS",
