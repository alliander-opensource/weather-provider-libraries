...
from weather_provider_libraries.__oldest__.data_classes.factor_related import ModelFactor
from weather_provider_libraries.__oldest__.data_classes.model_related import (
    ModelGeography,
    ModelIdentity,
    ModelProperties,
)
from weather_provider_libraries.__oldest__.data_classes.other import TimePeriod, UnitSystem
from weather_provider_libraries.__oldest__.data_classes.storage_related import StorageMode
    ...
    def __init__(self, init_folder: Path):
        ...
        if bbox_contents is not None and isinstance(bbox_contents, list) and len(bbox_contents) == 4:
            area_bounding_box = BBox(
                west=bbox_contents[0], south=bbox_contents[1], east=bbox_contents[2], north=bbox_contents[3]
            )
        else:
        ...
        self.factor_information = {
            factor_id: ModelFactor(
                identifier=factor_id, linked_eccodes_factor=factor_info["linkedECCODESFactor"], unit=factor_info["unit"]
            )
