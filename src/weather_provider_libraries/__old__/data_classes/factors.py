...
from weather_provider_libraries.utils.pint_utils import validate_pint_unit_string
...
class ModelFactor(BaseModel):
    ...
    def self_validate(self):
        """Check if the current factor is valid."""
        if len(self.id) < 1:
            raise ValueError("The factor id is an empty string!")

        validate_pint_unit_string(self.units)
