...
"""This module contains the pydantic_settings based class for the WPAS libraries."""
import re
import tempfile
import warnings
...
class ProjectSettings(BaseSettings):
    """This class embodies the project's settings.

    It is used to gather and validate the settings for the WPAS libraries component.
    """
    ...
    def validate_controller_sources(cls, value) -> list[str]:
        ...
        if (not isinstance(value, str) and
                not (isinstance(value, list) and all(isinstance(item, str) for item in value))):
            raise ValueError("The <PROJECT>_CONTROLLER_SOURCES environment variable must be a string. "
                             f"(Found: {type(value)})")
        ...
        if not bool(re.fullmatch(_SOURCE_ID_PATTERN, value)) and value != "autoloader":
            raise ValueError("There was an invalid value found for the <PROJECT>_CONTROLLER_SOURCES environment "
    ...
    def validate_storage_base_path(cls, value) -> Path:
        ...
        if value == Path(tempfile.gettempdir()):
            warnings.warn("The storage base path is set to the system's temporary directory. This is not recommended.")
