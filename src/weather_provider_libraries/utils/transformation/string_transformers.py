...
def parse_bytesize_to_easily_readable(byte_value: int | float, current_unit: str = "MB", decimals: int = 2) -> str:
    """Convert a number of bytes with a given unit into an easily readable string.

    Convert a number of bytes with a given unit into an easily readable string with the most appropriate format
    and the requested number of decimal places.
