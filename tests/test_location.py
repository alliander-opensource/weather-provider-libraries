...
@pytest.mark.parametrize(
    ...
    [
        ...
        (52.377956, 4.897070, CRS.from_epsg(4326), CRS.from_epsg(28992), 121626.0, 487900.0),
        (121626.0, 487900.0, CRS.from_epsg(28992), CRS.from_epsg(4326), 52.377956, 4.897070),
        # Arnhem: RD New to ETRS89 and back
        (121626.0, 487900.0, CRS.from_epsg(28992), CRS.from_epsg(23031), 629221, 5804980),
        (629221.0, 5804980.0, CRS.from_epsg(23031), CRS.from_epsg(28992), 121626, 487900),
    ],
)
def test_as_crs(x, y, from_crs, to_crs, expected_x, expected_y):
    ...
    # Act
    converted_location = geo_location_to_convert.as_crs(to_crs)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (121626.0, 487900.0, CRS.from_epsg(28992), "x=easting, y=northing"),
        # Arnhem: RD New to ETRS89 and back
        (121626.0, 487900.0, CRS.from_epsg(28992), "x=easting, y=northing"),
        (629221.0, 5804980.0, CRS.from_epsg(23031), "x=easting, y=northing"),
    ],
)
